...
async function obtenerMarcadoresPorIDLiga(req, res) {
    var idLiga = req.params.idLiga;
    var Equipos = await Equipo.find({ liga: idLiga })
    var Partidos = await Partido.find({ $or: [{ equipo_local: Equipos, equipo_visitante: Equipos }] });

    await Resultado_Partido.find({ $or: [{ partido: Partidos }] }).populate({
            path: "partido",
            populate: [{
                path: "equipo_local",
                model: "equipo"
